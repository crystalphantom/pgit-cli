  ...
  describe('addToGitExclude method', () => {
    ...
    describe('duplicate prevention', () => {
      it('should not add duplicate entries', async () => {
        ...
        const content = await fs.readFile(gitExcludePath, 'utf8');
        const occurrences = (content.match(new RegExp(testPath.replace('.', '\\.'), 'g')) || []).length;
...
`;
  ...
  describe('removeFromGitExclude method', () => {
    ...
    describe('input validation', () => {
      ...
      it('should throw GitExcludeValidationError for invalid characters', async () => {
        await expect(gitService.removeFromGitExclude('file\0.txt')).rejects.toThrow(GitExcludeValidationError);
      });
    });
  ...
  describe('exclude file creation and modification scenarios', () => {
    it('should create exclude file with proper structure', async () => {
      ...
      expect(lines).toContain('# pgit-cli managed exclusions');
...
`;
      ...
      expect(afterAddContent).toContain('# pgit-cli managed exclusions');
      ...
      expect(afterRemoveContent).toContain('# pgit-cli managed exclusions');
...
`;
      ...
      expect(afterRemoveContent).not.toContain('# pgit-cli managed exclusions');
      for (const pgitPath of pgitPaths) {
