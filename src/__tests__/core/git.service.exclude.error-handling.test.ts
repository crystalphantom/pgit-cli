...
describe('GitService Exclude Error Handling', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(__dirname, '..', '..', '..', 'test-temp', `git-exclude-error-${Date.now()}`);
  ...
  describe('Input Validation', () => {
    ...
    it('should handle null characters in paths', async () => {
      await expect(gitService.addToGitExclude('test\0file')).rejects.toThrow(GitExcludeValidationError);
    });
    ...
    it('should validate multiple paths in batch operations', async () => {
      const result = await gitService.addMultipleToGitExclude(['valid-path', '', 'test\0file', '   ']);
  ...
  describe('Batch Operation Error Handling', () => {
    ...
    it('should handle batch removal with mixed results', async () => {
      ...
      await fs.ensureDir(path.dirname(gitExcludePath));
      const initialContent = `# pgit-cli managed exclusions
file1.txt
file2.txt
file3.txt
`;
      ...
      const result = await gitService.removeMultipleFromGitExclude(['file1.txt', '', 'file2.txt', 'nonexistent.txt']);
